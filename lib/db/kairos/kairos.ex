defmodule KairosDatabase do
  ...
  def insert(host, port, metric, timestamp, value) do
    ...
    Logger.info fn -> "Insert ran in #{optime / 1_000_000} with response #{response |> elem(0)}" end
    {:ok, optime}
  end
  ...
  def read(host, port, metric, timestamp) do
    ...
    data = '{"start_absolute":1234,"end_absolute":1235,"metrics":[{"tags":{},"name":"localhost","limit":10}]}'
    args = [url, data, [{"Content-Type", "application/json"}]]
